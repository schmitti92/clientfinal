...
try{ ws = new WebSocket(SERVER_URL); }
  ...
  let diceRollAnim = { running:false, raf:0, t0:0, dur:0, final:0 };

  function animateDiceRoll(finalFace){
    if(!diceEl) return;
    // stop previous
    if(diceRollAnim.raf) cancelAnimationFrame(diceRollAnim.raf);
    diceRollAnim.running = true;
    diceRollAnim.final = finalFace;
    diceRollAnim.t0 = performance.now();
    // 2.0s - 2.5s (user wish)
    diceRollAnim.dur = 2000 + Math.random()*500;

    let lastTick = 0;

    const tick = (now)=>{
      const t = (now - diceRollAnim.t0) / diceRollAnim.dur;
      if(t >= 1){
        diceEl.dataset.face = String(diceRollAnim.final);
        diceEl.classList.remove("shake");
        diceRollAnim.running = false;
        diceRollAnim.raf = 0;
        return;
      }
      ...
      const interval = 45 + ease * 210; // becomes slower
      ...
      diceRollAnim.raf = requestAnimationFrame(tick);
    };
  ...
  function setDiceFaceAnimated(v){
    ...
    // always animate on server roll (even if same face twice)
    lastDiceFace = face;
    ...
    // roll simulation (2.0–2.5s) then land on server result
    animateDiceRoll(face);
  }
...
if(phase==="placing_barricade" && hit && hit.kind==="board"){
  ...
  // Würfel-Klick = Würfeln (Button bleibt als Fallback im DOM, aber UI nutzt den Würfel)
  if(diceEl){
