...
try{ ws = new WebSocket(SERVER_URL); }
  ...
  // ===== Big centered dice overlay (visual only, safe) =====
  let bigDiceT1=null, bigDiceT2=null, bigDiceT3=null;

  function clearBigDiceTimers(){
    if(bigDiceT1) clearTimeout(bigDiceT1);
    if(bigDiceT2) clearTimeout(bigDiceT2);
    if(bigDiceT3) clearTimeout(bigDiceT3);
    bigDiceT1=bigDiceT2=bigDiceT3=null;
  }

  function showBigDice(face, opts={}){
    if(!bigDiceEl) return;
    clearBigDiceTimers();

    const f = (face>=1 && face<=6) ? face : 0;
    bigDiceEl.dataset.face = String(f);

    // reset classes
    bigDiceEl.classList.remove("rolling","settle");
    void bigDiceEl.offsetWidth; // restart animations
    bigDiceEl.classList.add("show","rolling");

    const rollMs = Number(opts.rollMs ?? 1300);   // slow spin
    const settleMs = Number(opts.settleMs ?? 380);
    const holdMs = Number(opts.holdMs ?? 1000);   // (≈1s) visible after stop

    // Stop rolling -> settle (mini hop + recoil)
    bigDiceT1 = setTimeout(()=>{
      if(!bigDiceEl) return;
      bigDiceEl.classList.remove("rolling");
      bigDiceEl.classList.add("settle");
    }, rollMs);

    // End settle
    bigDiceT2 = setTimeout(()=>{
      if(!bigDiceEl) return;
      bigDiceEl.classList.remove("settle");
    }, rollMs + settleMs);

    // Hide
    bigDiceT3 = setTimeout(()=>{
      if(!bigDiceEl) return;
      bigDiceEl.classList.remove("show","rolling","settle");
      bigDiceEl.dataset.face = "0";
    }, rollMs + settleMs + holdMs);
  }

function setDiceFaceAnimated(v){
    if(!diceEl) return;

    // Show big centered dice while rolling (visual only)
    showBigDice(0, { rollMs: 1300, settleMs: 380, holdMs: 1000 });
    ...
    setTimeout(()=>{
      ...
      if(bigDiceEl) bigDiceEl.dataset.face = String(face);
