(() => {
  ...
  // ===== Game state =====
  let phase = "need_roll";            // need_roll | need_move | placing_barricade | moving | game_over
  let legalTargets = [];
  let placingChoices = [];
  ...
  // Per-piece hop when server sends move_step
  const pieceStepFx = new Map(); // pieceId -> {t0,dur}

  function markPieceStepFx(pieceId){
    pieceStepFx.set(String(pieceId), { t0: performance.now(), dur: 220 });
    ensureAnimLoop();
  }
...
try{ ws = new WebSocket(SERVER_URL); }
    ...
    ws.onmessage = (ev) => {
      ...
      if(type==="move_begin"){
        // Server-side step animation starting
        if(msg.action) queueMoveFx({ pieceId: msg.action.pieceId, path: msg.action.path, kickedPieces: [] });
        if(msg.state) applyRemoteState(msg.state);
        if(Array.isArray(msg.players)) setNetPlayers(msg.players);
        return;
      }
      if(type==="move_step"){
        // Each step already comes with the updated server state (piece moved one node)
        if(msg.state) applyRemoteState(msg.state);
        if(msg.action?.pieceId) markPieceStepFx(String(msg.action.pieceId));
        if(Array.isArray(msg.players)) setNetPlayers(msg.players);
        return;
      }
      if(type==="move_end"){
  ...
  function applyRemoteState(remote){
    ...
    if(st.turnColor && Array.isArray(st.pieces) && Array.isArray(st.barricades)){
      ...
      else if(ph==="moving") phase="moving";
  ...
  function updateTurnUI(){
    ...
    if(phase==="moving") turnText.textContent = `${PLAYER_NAME[c]} läuft…`;
    else turnText.textContent = state.winner ? `${PLAYER_NAME[state.winner]} gewinnt!` : `${PLAYER_NAME[c]} ist dran`;
    turnDot.style.background = COLORS[c];

    const isMyTurn = (netMode==="offline") ? true : (myColor && myColor===state.currentPlayer);
    const locked = (phase==="moving");
    rollBtn.disabled = locked || (phase!=="need_roll") || !isMyTurn;
    endBtn.disabled  = locked || (phase==="need_roll"||phase==="placing_barricade"||phase==="game_over") || !isMyTurn;
    if(skipBtn) skipBtn.disabled = locked || (phase==="placing_barricade"||phase==="game_over") || !isMyTurn;
  ...
  function drawStack(arr, x, y, r){
    ...
    // (Weg 1) hop/bounce on every server step
    // If the top piece in this stack is the moving one, lift it slightly so it's clearly in the foreground.
    let lift = 0;
    try{
      const pid = state?.pieces?.[p.color]?.[p.index]?.pieceId;
      if(pid && pieceStepFx.has(pid)){
        const fx = pieceStepFx.get(pid);
        const age = performance.now() - fx.t0;
        if(age >= fx.dur) pieceStepFx.delete(pid);
        else{
          const t = age / fx.dur;
          // smooth hop: up then down
          const s = Math.sin(Math.PI * Math.max(0, Math.min(1, t)));
          lift = - (r * 0.55) * s;
        }
      }
    }catch(_e){}
