(() => {
  ...
  function updateStartBtn(){
    ...
    const ready = (ws && ws.readyState===1);
    startBtn.disabled = !(netMode==="host" && isHost && ready && !!lastCanStart);
  }

  function renderReclaimUI(){
    if(!reclaimArea) return;
    reclaimArea.innerHTML = "";
    const me = rosterById.get(clientId);
    if(!(netMode==="host" && me?.isHost)) return;

    const offlineByColor = new Map();
    for(const p of lastNetPlayers){
      if(p?.color && p.connected===false) offlineByColor.set(p.color, p);
    }
    if(!offlineByColor.size) return;

    const claimables = lastNetPlayers.filter(p=>p && p.connected!==false && !p.color && !p.isHost);
    if(!claimables.length) {
      // Hinweis: erst wenn der Spieler ohne Token beigetreten ist (als Zuschauer), kann Host claimen.
      const div = document.createElement("div");
      div.className = "muted";
      div.textContent = "Reconnect-Notfall: Spieler ohne Farbe beitreten lassen, dann hier Slot zurückholen.";
      reclaimArea.appendChild(div);
      return;
    }
    ...
    wrap.innerHTML = `<div class="lbl" style="margin-bottom:6px;">Reconnect-Notfall (Host): Slot zurückholen</div>`;

    for(const [color, offP] of offlineByColor.entries()){
      ...
      title.textContent = `${PLAYER_NAME[color]} ist offline → zuweisen an:`;
      row.appendChild(title);

      for(const p of claimables){
        const b = document.createElement("button");
        b.className = "btn";
        b.textContent = p.name ? p.name : (p.id||"Spieler");
        b.addEventListener("click", ()=>{
          wsSend({ type:"claim_slot", color, toClientId: p.id, ts: Date.now() });
        });
        row.appendChild(b);
      }
