(() => {
  ...
  // ===== View / Fit-to-screen (wichtig für Tablet) =====
  ...
  function resize(){
    ...
    // Wenn das Canvas (noch) keine Größe hat (Tablet/Layout-Wechsel), nicht kaputt rechnen:
  ...
  function draw(){
    ...
    const rW=Math.max(16, board.ui?.nodeRadius || 20);

    // Radius skaliert mit Zoom (sonst überlappt alles, wenn das Board klein gefittet wird – Tablet!)
    const r=clamp(rW*view.s, 9, 28);
  ...
  function hitNode(wp){
    const rW=Math.max(16, board.ui?.nodeRadius || 20);
    const r=clamp(rW*view.s, 9, 28);
    // Trefferfläche in WORLD: (Pixel-Radius + Puffer) zurückrechnen
    const hitR=(r+18)/view.s;
