...
try{ ws = new WebSocket(SERVER_URL); }
  ...
  function queueMoveFx(action){
    ...
    const stepMs = 170;   // per-field travel time
    const holdMs = 45;    // tiny pause on each field (landing)
    const hopPx  = 16;    // jump height in px
    moveGhostFx = { color: color || "white", pts, t0: now, stepMs, holdMs, hopPx, dur: nseg * (stepMs + holdMs) };
  }
  ...
  function draw(){
    ...
    if(SHOW_MOVE_TRAIL && lastMoveFx && lastMoveFx.pts && nowFx - lastMoveFx.t0 < 900){
      ...
      ctx.stroke();
      ctx.restore();
    }// (7) ghost piece sliding along path (visual only) — A+B+D: hop + easing + landing
if(moveGhostFx && moveGhostFx.pts){
  const pts = moveGhostFx.pts;
  const nseg = pts.length - 1;
  const stepMs = moveGhostFx.stepMs || 170;
  const holdMs = moveGhostFx.holdMs || 45;
  const hopPx  = moveGhostFx.hopPx  || 16;

  const segTime = stepMs + holdMs;
  const total = nseg * segTime;
  const elapsed = nowFx - moveGhostFx.t0;

  if(elapsed >= 0 && elapsed < total){
    const f = Math.max(0, Math.min(1, elapsed / total)); // overall progress 0..1

    // which segment are we in?
    const segIdx = Math.min(nseg-1, Math.floor(elapsed / segTime));
    const within = elapsed - segIdx * segTime;

    // local progress inside this segment (0..1), then hold
    let localT = (within <= stepMs) ? (within / stepMs) : 1;
    ...
    let x = a.x + (b.x - a.x) * localT;
    let y = a.y + (b.y - a.y) * localT;

    // hop arc (A) — only while traveling, not during hold
    if(within <= stepMs){
      const hop = Math.sin(Math.PI * localT) * hopPx;
      y -= hop; // screen y goes down, so subtract to jump up
    }

    const col = COLORS[moveGhostFx.color] || moveGhostFx.color || 'rgba(255,255,255,0.9)';

    // landing micro-bounce (D) — during hold time right after landing
    let landK = 0; // 0..1
    if(within > stepMs && holdMs > 0){
      landK = Math.max(0, Math.min(1, (within - stepMs) / holdMs));
    }

    ctx.save();
    // fade a bit over total path
    ctx.globalAlpha = 0.80 * (1 - f*0.25);

    const baseR = 14;
    // a tiny "squash" on landing: big -> normal
    const rr = baseR * (1 + 0.18 * (1 - landK));
