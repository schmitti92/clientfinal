...
(() => {
  ...
  // FX animation loop (visual only)
  let rafMoveFx = null;
  function isFxActive(){
    const now = performance.now();
    const trailOn = !!(lastMoveFx && lastMoveFx.pts && (now - lastMoveFx.t0) < 900);
...
try{ ws = new WebSocket(SERVER_URL); }
    ...
    ws.onmessage = (ev) => {
      ...
      if(type==="assign"){
        ...
        // as source of truth: keep server assignment
        // request a fresh snapshot after reconnect / join
        ...
        return;
            ...
            if(type==="reset_done"){
        clearLocalState();
        showToast("Reset: neues Spiel kann gestartet werden.");
        updateTurnUI();
        draw();
        return;
      }
...
if(type==="snapshot" || type==="started" || type==="place_barricade"){
  ...
  function getSessionToken(){
    // v2: token is scoped per room + role to avoid accidental collisions
    // (e.g., host/client tests, multiple rooms)
    try{
      const rc = (roomCode || "").trim().toUpperCase() || "GLOBAL";
      const role = (netMode || "offline");
      const keyV2 = `barikade_sessionToken_v2_${rc}_${role}`;
      let t = localStorage.getItem(keyV2) || "";
      if(!t){
        // migrate old global token once (does not overwrite old key)
        const old = localStorage.getItem("barikade_sessionToken") || "";
        t = old || ("S-" + randId(16));
        localStorage.setItem(keyV2, t);
        if(!old) localStorage.setItem("barikade_sessionToken", t);
      }
      return t;
    }catch(_e){
      return "S-" + randId(16);
  ...
  function applyRemoteState(remote){
    ...
    if(st.turnColor && Array.isArray(st.pieces) && Array.isArray(st.barricades)){
      ...
      state = {
        ...
        winner: (server.winner ? String(server.winner) : null),
      ...
      else if(ph==="game_over") phase="game_over";
  ...
  function checkWin(){
    for(const c of PLAYERS){
      if(state.pieces[c].filter(p=>p.pos==="goal").length>= 1){ state.winner=c; return; }
    }
  }
  ...
  function draw(){
    ...
    const rect=canvas.getBoundingClientRect();
    if(!state){
      // Show a friendly message instead of a blank/frozen board
      ...
      ctx.fillText("Spiel nicht gestartet", rect.width/2, rect.height/2 - 10);
      ctx.font="14px system-ui";
      ctx.fillStyle="rgba(230,237,243,0.75)";
      ctx.fillText("Host: Spiel starten oder Save laden", rect.width/2, rect.height/2 + 16);
      ctx.restore();
      return;
    }
    const rect=canvas.getBoundingClientRect();
    ...
    // FX (foreground)
// (109) last move trail + (8) destination glow + (8) destination glow
  ...
  function onPointerDown(ev){
    ...
    if(!state){ toast("Spiel nicht gestartet / kein Spielstand"); return; }
    ...
    const isMyTurn = (netMode==="offline") ? true : (!!myColor && myColor===state.currentPlayer);
    if(netMode!=="offline" && (!myColor || !isMyTurn) && (phase==="placing_barricade" || phase==="need_move" || phase==="need_roll")){
