...
// ---- Animation ticker (needed so movement is smooth and not "only when something else happens") ----
let _animReq = 0;
function fxActive(){
  const now = performance.now();
  const ghostLive = !!(moveGhostFx && moveGhostFx.pts && (now - moveGhostFx.t0) < (moveGhostFx.dur||0));
  const trailLive = !!(lastMoveFx && lastMoveFx.pts && (now - lastMoveFx.t0) < 900);
  const landLive = !!(landFxs && landFxs.length);
  const movingLive = !!(movingPiecesFx && movingPiecesFx.size);
  return ghostLive || trailLive || landLive || movingLive;
}
function startAnim(){
  if(_animReq) return;
  const tick = () => {
    _animReq = requestAnimationFrame(tick);
    if(!fxActive()){
      stopAnim();
      return;
    }
...
function stopAnim(){
  ...
  function queueMoveFx(action){
    ...
    // Slower + more visible: ~360ms per step, capped.
    const dur = Math.min(2200, 420 + steps * 360);
    const weights = buildGhostWeights(pts);

    // keep existing trail/glow (nice), but ALSO animate the real piece position (no teleport feeling)
  ...
  function updateTurnUI(){
    ...
    const isBlocked = (phase==="anim");
    rollBtn.disabled = isBlocked || (phase!=="need_roll") || !isMyTurn;
    endBtn.disabled  = isBlocked || (phase==="need_roll"||phase==="placing_barricade"||phase==="game_over") || !isMyTurn;
    if(skipBtn) skipBtn.disabled = isBlocked || (phase==="placing_barricade"||phase==="game_over") || !isMyTurn;
