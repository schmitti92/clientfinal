...
(() => {
  ...
  function getConnByColor(){
    const byColor = Object.create(null);
    for(const p of (lastNetPlayers||[])){
      if(p && p.color) byColor[p.color] = !!p.connected;
    }
    return byColor;
  }
  ...
  function renderConnStatus(){
    ...
    const byColor = getConnByColor();
    const order = ["red","blue","yellow","green"];
    const parts = [];
    for(const c of order){
      if(Object.prototype.hasOwnProperty.call(byColor, c)){
        parts.push(`<span class="pill ${c}">${PLAYER_NAME[c]||c} ${byColor[c] ? "✓" : "✖"}</span>`);
      }
    }
    connStatusEl.innerHTML = parts.length ? parts.join(" ") : "–";
  }

  function computeWaitReason(){
    const assigned = (lastNetPlayers||[]).filter(p => p && p.color);
    const missing = assigned.filter(p => !p.connected).map(p => PLAYER_NAME[p.color]||p.color);
    if(missing.length) return `Warte auf ${missing.join(", ")} (Reconnect)`;
    if(assigned.length < 2) return "Warte auf 2 Spieler (Farben)";
    return "Alle verbunden";
  }

  function renderWaitReason(text){
    if(!waitReasonEl) return;
    waitReasonEl.textContent = (typeof text==="string" && text) ? text : computeWaitReason();
...
function updateHostToolsUI(){
    ...
    // Resume-Gate + Erklärung (keine Logik ändern – nur UI)
    if(resumeBtn){
      const st = state;
      const gateReason = (() => {
        if(netMode==="offline") return "Offline";
        if(!isMeHost()) return "Nur Host darf fortsetzen";
        if(!st || !st.started) return "Spiel läuft nicht";
        if(!st.paused) return "Spiel ist nicht pausiert";
        if(!netCanStart) return computeWaitReason();
        return "Bereit";
      })();

      const canResume = !!(show && st && st.started && st.paused && netCanStart);
      resumeBtn.disabled = !canResume;
      resumeBtn.title = canResume ? "Spiel fortsetzen" : gateReason;

      // Sidebar-Text: zeigt sofort, worauf gewartet wird
      renderWaitReason(gateReason.startsWith("Warte") ? gateReason : "");
    }
  }
...
if(phase==="placing_barricade" && hit && hit.kind==="board"){
  ...
  // Host: resume after reconnect pause
  if(resumeBtn){
