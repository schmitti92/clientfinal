...
(() => {
  ...
  const reactParty = $("reactParty");
  const reactShock = $("reactShock");
  ...
  function nameForColor(color){
    try{
      ...
      for(const p of lastNetPlayers){
        if(p && p.color===c && p.connected!==false) return p.name || PLAYER_NAME[c] || c;
      }
      for(const p of lastNetPlayers){
        if(p && p.color===c) return p.name || PLAYER_NAME[c] || c;
      }
    }catch(_e){}
    return PLAYER_NAME[color] || String(color||"");
  }
  ...
  // Online-only mini FX
  let barrPlaceFx = null; // {nodeId, t0}
  let barrSuckFx  = null; // {nodeId, t0, dur}
  let kickFx = [];        // [{nodeId, t0, dur}]
  let _pendingPlaceNode = null;
  let barricadePlaceFx = null; // {nodeId,t0,dur}
  let barricadePickupFx = null; // {x,y,t0,dur}
  let kickFx = null; // {x,y,t0,dur}
...
try{ ws = new WebSocket(SERVER_URL); }
    ...
    ws.onmessage = (ev) => {
      ...
      if(type==="roll"){
        ...
        if(typeof msg.value==="number") { setDiceFaceAnimated(msg.value); toast(`🎲 Wurf: ${msg.value}`); }
        if(msg.state){
          applyRemoteState(msg.state);
          writeHostAutosave(msg.state);
        }
      ...
      if(type==="move"){
        ...
        if(msg.action){
          ...
          try{
            ...
            const path = Array.isArray(act.path) ? act.path : [];
            const landed = path.length ? String(path[path.length-1]) : "";
            if(act.pickedBarricade && landed){ startBarricadeSuckFx(landed); toast("🧱 Barikade eingesammelt"); }
            if(Array.isArray(act.kickedPieces) && act.kickedPieces.length && landed){ startKickFx(landed); toast("🚀 Figur rausgeworfen!"); }
          }catch(_e){}
        }
        if(msg.state){
          applyRemoteState(msg.state);
      ...
      if(type==="react"){
        const em = String(msg.emoji||"").slice(0,8);
        const from = msg.from || (msg.color ? PLAYER_NAME[msg.color] : "Spieler");
        if(em) {
          toast(`${em} ${from}`);
          try{ spawnReaction(em); }catch(_e){}
        }
        return;
      }
      if(type==="pong") return;
...
function toast(msg){
  ...
  function showReactionFloat(emoji, x, y){
  ...
  function showReaction(emoji, name){
    ...
    showReactionFloat(emoji, cx, cy);
    if(name) toast(`${emoji} ${name}`);
    else toast(`${emoji}`);
  }

  function spawnReaction(emoji){
    try{
      const pill = document.querySelector(".turnPill");
      const rect = pill ? pill.getBoundingClientRect() : null;
      const x = rect ? (rect.left + rect.width/2) : (window.innerWidth/2);
      const y = rect ? (rect.top + 10) : 24;

      const el = document.createElement("div");
      el.className = "reactionPop";
      el.textContent = String(emoji||"").slice(0,8);
      el.style.left = x + "px";
      el.style.top = y + "px";
      document.body.appendChild(el);
      setTimeout(()=>{ try{ el.remove(); }catch(_e){} }, 950);
    }catch(_e){}
  }
  ...
  function setDiceFaceAnimated(v){
    ...
    // show "…" shortly before result
    diceEl.classList.add("pending");
    ...
    setTimeout(()=>{
      ...
      if(diceEl) diceEl.classList.remove("pending");
    }, 280);
  }
  ...
  function startBarricadePlaceFx(nodeId){
    if(!nodeId) return;
    barrPlaceFx = { nodeId: String(nodeId), t0: performance.now() };
    requestDraw();
  }
  function startBarricadeSuckFx(nodeId){
    if(!nodeId) return;
    barrSuckFx = { nodeId: String(nodeId), t0: performance.now(), dur: 520 };
    requestDraw();
  }
  function startKickFx(nodeId){
    if(!nodeId) return;
    kickFx.push({ nodeId: String(nodeId), t0: performance.now(), dur: 520 });
    // keep list small
    if(kickFx.length > 6) kickFx = kickFx.slice(-6);
    requestDraw();
  }
function queueMoveFx(action){
    ...
    const stepMs = 260; // slower, more boardgame-feel
    const totalMs = Math.min(3200, Math.max(520, steps * stepMs));
  ...
  function updateTurnUI(){
    ...
    turnText.textContent = state.winner ? `${nameForColor(state.winner)} gewinnt!` : `${nameForColor(c)} ist dran`;
    ...
    // Dice pulse when it's your turn to roll
    if(diceEl){
      const pulse = isMyTurn && (phase==="need_roll") && !state.winner && !state.paused;
      diceEl.classList.toggle("myturn", !!pulse);
    }
...
if(!node) return false;
  ...
  function movePiece(move){
    ...
    if(enemies.length){
      try{
        const n = nodeById.get(String(toId));
        if(n) kickFx = { x:n.x, y:n.y, t0: performance.now(), dur: 520 };
      }catch(_e){}
      toast("🚀 Gegner rausgeworfen!");
    }
    ...
    if(landsOnBarr){
      try{
        const n = nodeById.get(String(toId));
        if(n) barricadePickupFx = { x:n.x, y:n.y, t0: performance.now(), dur: 420 };
      }catch(_e){}
  ...
  function drawBarricadeIcon(x,y,r,nodeId){
    ctx.save();

    // mini-bounce on place (visual only)
    try{
      const bfx = barricadePlaceFx;
      if(bfx && bfx.nodeId===nodeId){
        const age = performance.now() - (bfx.t0||0);
        const dur = bfx.dur || 420;
        if(age <= dur){
          const t = Math.max(0, Math.min(1, age/dur));
          const s = 1 + 0.32 * (Math.sin(t*Math.PI) * (1 - t*0.25));
          ctx.translate(x,y);
          ctx.scale(s,s);
          ctx.translate(-x,-y);
        }else{
          barricadePlaceFx = null;
        }
      }
    }catch(_e){}
  ...
  function draw(){
    ...
    /* (online) barricade suck fx */
    try{
      if(barrSuckFx && barrSuckFx.nodeId){
        const t = performance.now() - barrSuckFx.t0;
        const dur = barrSuckFx.dur || 520;
        if(t < dur){
          const n = nodeById.get(String(barrSuckFx.nodeId));
          if(n){
            ...
            ctx.beginPath();
            ctx.arc(sp.x, sp.y, 10*a, 0, Math.PI*2);
            ctx.fill();
            ctx.restore();
          }
        } else {
    ...
    /* (online) kick blink fx */
    try{
      if(kickFx && kickFx.length){
        const nowK = performance.now();
        kickFx = kickFx.filter(k => (nowK - k.t0) < (k.dur||520));
        for(const k of kickFx){
          const n = nodeById.get(String(k.nodeId));
          if(!n) continue;
          const sp = worldToScreen(n);
          const age = nowK - k.t0;
          const dur = k.dur||520;
          const a = Math.max(0, 1 - age/dur);
          const pulse = 0.6 + 0.4*Math.sin((age/dur)*Math.PI);
          ctx.save();
    ...
    // Extra FX: kick blink + barricade pickup/place bounce
    ...
    // kick blink (opponent was kicked here)
    if(kickFx && nowFx - kickFx.t0 < (kickFx.dur||520)){
      const age = (nowFx - kickFx.t0);
      const a = Math.max(0, 1 - age/(kickFx.dur||520));
      const end = kickFx;
      const sp = worldToScreen({x:end.x, y:end.y});
      ctx.save();
      ...
      ctx.stroke();
      // little "fly away" particles
      ctx.globalAlpha = 0.65 * a;
      for(let i=0;i<5;i++){
        const ang = (i/5)*Math.PI*2 + 0.6;
        const dx = Math.cos(ang)*(18 + age*0.03);
        const dy = Math.sin(ang)*(10 + age*0.02) - age*0.08;
        ctx.beginPath();
        ctx.arc(sp.x+dx, sp.y+dy, 4, 0, Math.PI*2);
    ...
    // barricade pickup "suction"
    if(barricadePickupFx && nowFx - barricadePickupFx.t0 < (barricadePickupFx.dur||420)){
      const age = (nowFx - barricadePickupFx.t0);
      const a = Math.max(0, 1 - age/(barricadePickupFx.dur||420));
      const sp = worldToScreen({x:barricadePickupFx.x, y:barricadePickupFx.y});
      const k = 0.35 + 0.65*a;
      ctx.save();
      ctx.globalAlpha = 0.65 * a;
      ...
      ctx.lineWidth = 4;
      ctx.beginPath();
      ctx.arc(sp.x, sp.y, 24*k, 0, Math.PI*2);
      ctx.stroke();
      ctx.restore();
    }
...
const r=Math.max(16, board.ui?.nodeRadius || 20);
    ...
    for(const n of board.nodes){
      ...
      if(n.kind==="board"){
        ...
        // slightly darker houses for contrast
        const base = COLORS[n.flags?.houseColor]||COLORS.node;
        fill = base;
        try{
          // if base is rgb/rgba, apply a dark overlay later
        }catch(_e){}
      }
      ...
      if(n.kind==="house"){
        // house fields slightly darker for better contrast
        ctx.save();
        ctx.fillStyle = "rgba(0,0,0,0.18)";
        ctx.beginPath(); ctx.arc(s.x,s.y,r,0,Math.PI*2); ctx.fill();
        ctx.restore();
      ...
      if(n.kind==="board" && state.barricades.has(n.id)){
        drawBarricadeIcon(s.x,s.y,r, n.id);
      }
    }
    ...
    if(moveAnim){
      ...
      if(t >= moveAnim.totalMs){
        ...
        const gg = ctx.createRadialGradient(sp.x - rr*0.25, yHop - rr*0.35, rr*0.2, sp.x, yHop, rr*1.2);
        gg.addColorStop(0, "rgba(255,255,255,0.35)");
        gg.addColorStop(0.45, col);
...
if(phase==="placing_barricade" && hit && hit.kind==="board"){
  ...
  if(netMode!=="offline"){
    barricadePlaceFx = { nodeId: hit.id, t0: performance.now(), dur: 420 };
  ...
  // ===== Buttons =====
  reactParty && reactParty.addEventListener("click", ()=>{
    if(netMode==="offline"){ toast("🎉"); spawnReaction("🎉"); return; }
    if(!ws || ws.readyState!==1){ toast("Nicht verbunden"); return; }
    wsSend({type:"react", emoji:"🎉", ts:Date.now()});
  });
  reactShock && reactShock.addEventListener("click", ()=>{
    if(netMode==="offline"){ toast("😱"); spawnReaction("😱"); return; }
    if(!ws || ws.readyState!==1){ toast("Nicht verbunden"); return; }
    wsSend({type:"react", emoji:"😱", ts:Date.now()});
  });
